...
def replaceFileToTuple(fname):
    ...
    replStrgLst = []
    for line in fStrg.splitlines():
        replStrgLst = replStrgLst + [tuple([strg.strip() for strg in line.split(b":")])]
    return replStrgLst
...
def removeStopwordsFromFiles(fdir, odir, stops):
    ...
    for fname in fnames:
        ...
        for line in fStrg.splitlines():
            ...
            notInStops.append(b" ".join([w for w in lineLst if w.lower() not in stops]))
            #print(stops)
        ...
        with open(odir+os.sep+tail, "wb") as f:
            f.write(b"\n".join(notInStops))
